...
import { Plus, History, Music, Search, ExternalLink, Heart, Users, Calendar, Trash2, Play, Pause } from 'lucide-react';
...
export const MusicRequestsSection: React.FC<MusicRequestsSectionProps> = ({
  ...
  const handleTrackSelect = async (track: SpotifyTrack) => {
    ...
    try {
      ...
      setSubmitSuccess(`🎉 "${track.name}" wurde zur Playlist hinzugefügt!`);
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log(`🔗 Adding from URL: ${urlInput}`);
      await addMusicRequestFromUrl(urlInput.trim(), userName, deviceId, message.trim() || undefined);
      
      setSubmitSuccess('🎉 Song wurde zur Playlist hinzugefügt!');
      setShowRequestModal(false);
      setSearchQuery('');
      setSearchResults([]);
      setMessage('');
